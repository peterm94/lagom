...
class BumpMoveSystem extends System
{
    ...
    update(delta: number): void
    {
        this.runOnEntities((entity: Entity) => {
            entity.transform.position.x = (entity.transform.position.x + delta / 40) % 240;
        });
    }
}
...
class Conveyor extends Entity
{
    ...
    {
        super.onAdded();
        this.addComponent(new Sprite(cookingSheet.texture(0, 96, 240, 32)));
...
export class LobsterMinigame extends Entity
{
    onAdded(): void
    {
        ...
        this.addChild(new Chef("chef", 128));
